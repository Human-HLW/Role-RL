...
# Initialize dictionaries to store output, scores, costs, and rewards for different tasks.
output_dict = {}
score_dict = {}
score_dict_old = {}   # without gpt-4o mini
cost_dict = {}
cost_dict_old = {}   # without gpt-4o mini
reward_dict = {}
reward_dict_old = {}   # without gpt-4o mini
...
iters = 50000  # Total number of rounds
change_hardness_last = 500   # Number of rounds in each task for illustration
prob_change_hardness = 0.03  #
print_limit = 500
Hardness = []   # Difficulty history

# Epsilon values for exploration-exploitation trade-off in different phases.
epsilon = 0.03  # ----------
epsilon = float(epsilon)
epsilon_test_1 = epsilon
epsilon_test_2 = epsilon
epsilon_test_3 = epsilon
epsilon_test_4 = epsilon
...
# List of LLMs with their names, input and output cost per 1M tokens
...
c1_judger = 7 / 1000000    # input cost per token
c2_judger = 21 / 1000000    # output cost per token
weight_list = [0, 0, 0, 0, 1, 0]     ### indicate which LLMs are the board members

length_input = 25000   # fixed for a close comparison between LLMs in different roles and tasks.
length_output = 25000

File_path = ['Task2.txt', 'Task3.txt', 'Task1.txt']   # task file name
English = False

parameter_file = 'parameters.txt'   # store the print limit

gamma = 0  # Discount factor
n_LLMs = len(LLMs)   # number of actions
n_states = n_LLMs * 2   # number of states (success/failure for each LLM)

# Try to load existing output dictionaries by Gemini and Claude from file.
try:
...
# Initialize score, cost, and reward dictionaries
...
# Create copies of LLM lists for different roles.
...
# Initialize lists to store cost and reward data
...
# cost table in the same dimension
...
# Initialize lists to store LLM choices
LLM_TOPic_finder = []
LLM_TOPic_locator = []
LLM_RELationship_checker = []
LLM_CONtent_organizer = []

Markov_end = []   # List to store Markov chain end states

# calculate the Intersection over Union (IoU) of two intervals of sentence index
def iou(box1, box2):
    try:
        a1, b1 = box1
        a2, b2 = box2
...
# find indices of minimum values in a matrix (find an LLm to be replaced with gpt-4o mini)
...
# update the weight of the board members
...
# query an LLM and get its response
...
# query a board member to judge the correctness of an LLM's response
...
def relationship_checker(LLM_index, a):
    ...
    for item_index, the_item in enumerate(a[:-1]):
        ...
        if True:
            ...
            # If the response is "No" or "否", no further action is needed
            if "No" in result or "否" in result:
                # print("No need to combine.") if iter > (iters - print_limit) or iter < print_limit else None
                pass
            else:
                # If the response is "Yes" or "是", ask the LLM to merge the items
                if English:
...
# define the role of topic_locator with its output and reward
...
# define the role of topic_finder with its output and reward
...
# define the role of content_organizer with its output and reward
...
# call each role and update by reinforcement learning
def update(task, *args):
    ...
    # Determine which role is being updated and set corresponding variables
    if task == "topic_finder":
        my_fun = topic_finder  # Function to call for this task
    ...
    # Initialize dictionaries for the current file if not already present
    ...
    # when the Q-table is initialized (all zeros)
    if np.all(my_Qtable == 0):
        ...
        for i in range(n_LLMs - 1, -1, -1):
            ...
            # Update dictionaries and histories
        ...
        my_LLM_new = np.argmax(Reward)   # select the next action
    ...
    # when the Q-table is not all-zeros
    else:
        if np.random.rand() <= float(epsilon):
            my_LLM_new = np.random.choice(n_LLMs)   # explore

        else:
            my_LLM_new = np.argmax(my_Qtable[my_state])  # exploit
...
for iter in range(iters):
    ...
    elif iter <= iters - change_hardness_last * 3:  # -----------------tough (Task 1)
        Hardness.append(3)
        Epsilon.append(epsilon)
        the_file = File_path[len(File_path) - 1]
        recording = 1

    elif iter <= iters - change_hardness_last * 2:  # ----------------easy (Task 2)
        epsilon = epsilon_test_2
        Hardness.append(0)
        Epsilon.append(epsilon)
        the_file = File_path[0]

    elif iter <= iters - change_hardness_last * 1 + 10:  # ------------------medium (Task 3)
        epsilon = epsilon_test_3
        Epsilon.append(epsilon)
        Hardness.append(2)
        the_file = File_path[len(File_path) - 2]

    else:  # ------------------------------------------------gpt-4o mini added
        epsilon = epsilon_test_4
        if first_time:
    ...
    while 1:
        ...
        # update related to topic_finder
        ...
        # organize the sentences for each item
        ...
        # update related to content_organizer
